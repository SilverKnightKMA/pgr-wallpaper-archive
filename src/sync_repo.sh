...
# --- 0. Free disk: remove raw downloads (already processed into images/) ---
echo "Freeing disk before sync..."
while IFS= read -r id; do
  rm -rf "$REPO_DIR/branches/$id/downloads"
done <<< "$SERVERS"
echo "  Raw downloads removed. Disk: $(df -h / | tail -1)"
...
if git ls-remote --exit-code origin "$WALLPAPERS_BRANCH" >/dev/null 2>&1; then
  ...
  # Existing images in desktop/ and mobile/ are NOT downloaded or checked out
  git -C "$WP_DIR" sparse-checkout init --cone
...
# --- 4. Generate previews for NEW images only ---
# (Existing images' previews are already on the preview branch;
#  we skipped LFS smudge so existing files in WP_DIR are just pointers)
echo ""
echo "Generating previews for new images..."
mkdir -p "$PV_DIR/previews"
# Only process newly added images from branches/<server>/previews/
# which were already generated by process_images.py in the main workflow
...
if [ -z "$(git -C "$WP_DIR" status --porcelain)" ]; then
  ...
  while [ -n "$(git -C "$WP_DIR" status --porcelain)" ]; do
    ...
    # Prune LFS cache to free disk space after each push
    git -C "$WP_DIR" lfs prune 2>/dev/null || true
